...
- 默认 Plan 只制作代表性 HTML/CSS 布局样段，不初始化 HF、GSAP、Three 或全片时间线。Draft 复用已确认的文字、tokens、样式和有用代码，再实现完整工程；显式选择的 executable Plan 才按完整预演合同执行。
...
显式 executable Plan 及包含混合效果的 Draft 应实际播放、暂停、拖动、倒跳、再播放；固定环境按 `[t3, 0, t1, t3]` 抓帧比较同一时刻，并实际渲染短片核对 DOM 与 WebGL 同帧。验证全 Scene 覆盖、衔接、资源就绪、切换释放和正式画布阅读。默认 layout Plan 只验证静态样段的真实内容承载，不以它证明动效或渲染通过。Mock 或静态检查不算 Draft 渲染通过；技术样例不算真实生产复用。无关公共库更新不得使已冻结 Work 失效。
