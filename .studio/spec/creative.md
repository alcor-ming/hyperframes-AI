...
- Hero 与字幕条都从选定视频帧图片的底部向上裁切，不查找或保留源视频中的字幕；只由工作流绘制批准的中文文案。
- 1440x1920 画布中所有面板固定使用中文 42px，不显示原文；Hero 固定占 52%，3 至 4 条支撑字幕按文本实测高度动态分配其余 48% 且无间隙，必要时把水平边距从 6% 收到最低 3%。文字后的黑底使用 alpha 185，降低透明度以提高可读性；整图保留至少 40% 的无字幕视觉空间。
