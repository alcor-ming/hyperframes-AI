...
- 视频工作流的 `RESEARCH.md` 与 `SCRIPT.md` 采用同一共享/分支来源合同，记录 `script_revision`、Research Revision；按完整段落及现有 Scene / Anchor 保存事实、实例、素材依据、使用边界和真实待补问题。共享源变化需检查所有引用 Variant 的实际影响，不自动沿用旧接受。播客图文的 `RESEARCH.md` 不使用 Script Revision，只服务当前获批文章方案。
...
- 逐字口播只引用 Script Anchor；Research 保存有来源的事实与候选材料，Plan 的上屏信息表唯一维护实际采用的屏幕表达及来源，不为等义编辑回写第二份 Research 文案。原视频或截图内文字归素材，默认不全文抄写；“步骤一”“输入”“结果”等无独立主张的表现标签归工程，“效率提升十倍”等独立主张必须有相应内容依据。
- 主模型对最终 Scene 负责。换行、层级、分组、出现顺序及表现标签直接在样段或工程调整；删除重复措辞、等义精简或改成流程短语时同步 Plan 的实际上屏表达，不重新联网、不增加文字审批。是否等义以观众得到的结论不变为准；因果、比较对象、必要数值、单位、归属或证据边界变化属于实质修改，先回权威来源核对。
...
- 工程中的信息文字是内容呈现，不是独立改稿入口。修改同步 Plan 的实际上屏表达；新事实或改变原意才回 Script/Research 等权威来源。Plan 按完整理解单位记录信息 ID、来源、理解目标、实际表达、视觉职责、声画分工与揭示/阅读边界，不复制 Script/Research 全文或建立另一套文字数据库。
- 信息筛选和去重按 `.studio/workflow.md`：先选有效主题信息，制作约束不入正文，无增量重复删除，可选说明为空即省略；必要来源、对象、条件与导航保留。Studio 改稿同步 Plan 的实际上屏表达；新增事实、改变原意或证据边界时回权威来源，不维护第二套独立正文。
...
- 上屏信息表保存实际表达的唯一正文；Scene 节拍表只引用信息 ID、Script cue 和素材位置，不再抄写正文。按时间依据、观看任务、屏幕状态/焦点、状态变化及语义/阅读保护编排进入、阅读、变化、交接和退出，行数随实际变化；证据页可为单一持续阅读状态。
- Scene 给出视觉目标、对象与空间关系、阅读安排、素材用途与裁切意图。完整定义、引文及关键条件可以保留；已读信息可以移动、归组或退出，不能在读完前遮挡或清除。复杂场景才拆 Beat，不强制固定状态数。
