...
6. 共享已确认主题，局部布局仅在明确匹配的 Scene 复用；优先实现不确定镜头，再逐 Scene 补齐结构、动作与媒体。默认在官方 Studio 查看工程，登记版本用隔离审阅副本。主模型直接处理换行、分组和等义精简并同步 Research 采用条目；删除核心信息或改变结论才提交局部取舍。运行完整 Draft QA 并注册 Draft。
