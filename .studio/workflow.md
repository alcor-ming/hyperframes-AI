...
概念核查不能替代实际需要的对象与素材依据：需要具体实例、原生画面或操作结果才能解释时，定点补充可辨认对象、可采用内容、证据位置和必要使用条件，不要求每条研究都配图。采用信息原位写明当前取舍；旧事实审阅意见留在既有问题区或资料依据中并标明历史用途，不靠文首总覆盖规则保留相反的执行指令。未解决的事实冲突按既有局部确认处理，不自动改原音、已批准观点或添加纠错画面。
