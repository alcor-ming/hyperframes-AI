...
创建视频对象前先按 Router 区分讨论、继续现有对象、同目标独立 Variant、临时实验和新生产目标；只在用途仍有实质歧义时先问。新视频 Work 显式传 `--purpose standard|ip|test`；生产用途需要有效系列，可暂不指定账号、不创建 Variant，先准备共享内容，缺账号不等于实验。明确制作账号时可一次创建 Work 与对应 Variant；新生产 Variant 仍须绑定已登记账号。实验按用途命名、不绑定账号，准确账号设置只作参考快照。同账号可有多个平级 Variant，不用账号名代替准确版本选择。未指定叙事模式默认 `text-led`，不自动选择旧 Profile。账号设置及主题版本/参数在 Variant 创建时冻结，不随后续默认值更新漂移。

Work 级内容准备、列表、状态与切换不强制选 Variant。制作命令先用显式选择，否则只用属于该 Work 的有效当前选择或唯一候选；多候选返回候选，不按 main、创建顺序或账号优先级代选。Current 不是主版本，不跨 Work 沿用；零 Variant 时先创建制作版本，才能制作、预览或 Finalize。

仅当明确要求已归档生产 Work 保持只读、以新实体继续同一期时使用继任入口，指定准确源 Work、Variant、接受版本和新账号。新实体保留系列号与来源证据，不改变高水位，不继承接受或 Final；旧 ID 仍指旧实体，系列号查当前对象，历史查完整链。普通修订与账号适配不新建继任。
...
新视频 Work 的 Script/Research 默认位于 `shared/`，零 Variant 也能准备；有 Variant 时按其 `shared_inputs` 定位。各 Variant 拥有独立 Plan、工程和接受/Final，新版本默认采用共享来源，不隐式继承 main。显式 `variant add --from <id>` 建内容分支，旧 Variant-local 文稿位置保持兼容，不复制或重写历史来适配共享。
...
Script 稳定后用 `work --work <id> name "<核心主题>"` 补语义标题，不改身份或目录。Research 先理解完整段落，再按部分/Anchor 记录事实、实例、素材依据、使用边界和真实缺口；已有材料足够即复用，不强制配图或联网。它不规定布局、裁切或动画，不是第二份脚本或必上屏清单。具体材料缺口才定点补研究，不重开全轮或增加 Research 审批。事实冲突只提交相关影响，不擅改原音或阻塞无关部分。
...
视频 `required_variants` 只表示本期明确交付集合，不表示主版本。零 Variant 或空集合不算全部完成；交付范围未明确时按版本报告，一个 Final 不代表整期完成。旧交付集合、main ID、冻结配置与接受/Final 保留兼容。
