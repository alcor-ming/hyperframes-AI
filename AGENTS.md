...
Draft 与 test-work 不导出视频，包括带声短段、离线逐帧编码和预渲染镜头。只有生产 Variant 的 Finalize 交付链可生成视频；测试接受不变成生产接受。Studio 受限时记录未验证项并交工具缺口，不以导出绕过。软归档不授予接受、交付或删除权限，不搬目录、不清理依赖；旧物理归档保留兼容，唯 RC2 获准的旧视频 Work 可经专用 dry-run/apply 一次性迁移 ID、目录叶名和标题。日常改名或换系列不再改目录/ID，播客与冻结历史不迁移。
