...
- `ANIMATION_PLAN.md`：视觉语义与用户决策真源；与同源可播放预演一起确认，正式 Draft 前批准。Work 场景源码与 Binding 是可执行真源。
...
- 创建或复审 Animation Plan 时使用 `hyperframes-anti-ppt` 检查信息完整、阅读窗口、关系与动效适配；不强制统一场景公式。结果合并进现有 `ANIMATION_PLAN.md`，不新增文件或审批门。组件的检索、版本、Slots 与安装仍由视频工作流或组件库能力负责。
- Research 完成后，创建 Animation Plan 的同时调用 DBS 完成 `PACKAGE.md`；只保留最终标题、封面文字、一句话简介和内容概括。
- Script / Research 按现有路径就绪后，Plan 批准前允许创建 Work-local HTML、候选 Binding 和必要动效，展示所有 Scene 的实际文字、核心行为与交接；缺失关键素材必须标明。只做决定方向所需的预演，不进行无关高成本精修，不改公共冻结资产。
- 用户一次确认 `ANIMATION_PLAN.md` 与对应预演源码快照，不增加 Motion Plan 审批；正式 Draft 继续同源实现，不重写已确认场景。Plan 接受不写入 `accepted_preview`，不放行 Final。
- Draft 提交用户前使用 `hyperframes-anti-ppt` 复审成片或代表性关键帧；`PASS` 只是 QA 结果，不替代 Draft 接受。
- Final 必须从用户接受的 Draft 源码快照继续。
- 只在 Scene 数量、顺序、视觉目标、Hero State、Template、Profile 或文案结构发生实质变化时重新确认受影响部分。反馈绑定预演/Draft 版本、Scene 与必要时间范围，只修改该范围及必要交接，保留未受影响场景与原 Accepted Draft / Final 快照。
