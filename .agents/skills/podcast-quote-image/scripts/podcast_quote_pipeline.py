...
def validate_candidates(transcript_path: Path, candidates_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for candidate in candidates:
        ...
        if not isinstance(images, list) or len(images) not in range(8, 13):
            raise PipelineError(f"Candidate {candidate_id} must contain 8 to 12 image groups")
        ...
        for image_index, image in enumerate(images, 1):
            ...
            if not isinstance(units, list) or len(units) < 4:
                raise PipelineError(f"Candidate {candidate_id}/{image_id} requires 1 Hero and at least 3 supports")
            ...
            for unit_index, unit in enumerate(units, 1):
                ...
                for item in source_ids:
                    image_indexes = used_segments.setdefault(item, set())
                    image_indexes.add(image_index)
                    if len(image_indexes) > 2 or max(image_indexes) - min(image_indexes) > 1:
                        raise PipelineError(
                            f"Candidate {candidate_id} reuses a source segment outside two adjacent image groups"
                        )
...
def validate_selection(transcript_path: Path, selection_path: Path) -> tuple[dict[str, Any], dict[str, dict[str, Any]]]:
    ...
    if not isinstance(groups, list) or len(groups) not in range(8, 13):
        raise PipelineError("Approved article requires 8 to 12 image groups")
    for index, group in enumerate(groups, 1):
        if not isinstance(group, dict) or group.get("id") != f"g{index:02d}":
            raise PipelineError("Approved image groups must be ordered g01 through g12")
        if not isinstance(group.get("units"), list) or len(group["units"]) < 4:
            raise PipelineError(f"{group.get('id')} requires 1 Hero and at least 3 supports")
...
def command_extract(args: argparse.Namespace) -> None:
    ...
    if not isinstance(groups, list) or len(groups) not in range(8, 13):
        raise PipelineError("Aligned article requires 8 to 12 image groups")
