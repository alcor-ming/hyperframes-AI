...
Open Design 中的组合预览、缺失 HTML 的孤立 artifact 与 `image.png` 不进入本次公共化范围。

## Open Design 与生产真源

- Open Design `Hyperframes` 项目保存设计原稿。
- 集中 Gallery 覆盖 40 个 Component Ratio Releases 与 2 个 Background Ratio Releases。
- 用户逐项通过或退回；42 项全部通过后才允许整组入库。
...
- 三套 Profile、二十个公共 Component Family、一个 Background Family 和两个画幅已锁定。
- Open Design 中存在二十个当前组件原稿与 `rse-functional-background` 原稿。
- 新旧 Work 的兼容边界已明确。
...
- Background 的最终生产 ID。
- 42 个 Gallery artifact 的最终文件名、revision/hash 和视觉细节。
...
- Open Design 原稿足以还原二十个 `4x3` 组件，而无需依赖旧 Work 私有内容。
- 两个画幅能维持一致 Slot Schema；若某一画幅需要不同输入合同，应拆成新的 Component Family。
- 三套 Profile 的最小 token 能覆盖当前二十个组件与背景的主题需求。
...
- 全组合 QA 爆炸：只固定三套 Profile x 42 个 Ratio Releases 的主题冒烟，组件语义与 seek 检查按 Release 独立执行。
...
| 范围/非目标 | 只重构 3 Profile、20 Component Families、1 Background Family；不公共化组合预览、缺失 HTML 的孤立 artifact 与 `image.png`。 |
| 验收 | 42 项 Gallery、三 Profile 主题切换、跨画幅 Slot 一致、seek/像素/hash/离线渲染。 |
...
| Q-009 | 本轮包含哪些公共组件？ | 只包含当前二十个明确命名的公共 Family。 | 已决定 |
| Q-010 | 是否公共化范围外的 Open Design 原型？ | 否；组合预览、缺失 HTML 的孤立 artifact 与 `image.png` 继续排除。 | 已决定 |
| Q-011 | 是否同时交付两个画幅？ | 是，20 个组件和 1 个背景都交付 `4x3 + 16x9`。 | 已决定 |
| Q-012 | 是否迁移全部 Profile？ | 是，三套 Profile 一次性迁移。 | 已决定 |
| Q-013 | Open Design 如何验收？ | 集中 Gallery 展示 42 项，逐项验收，整组通过后入库。 | 已决定 |
...
- 只有二十个明确命名的 Component Family 进入公共化路径。
