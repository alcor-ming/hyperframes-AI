...
开发端固定使用 `release deploy --root <Windows-root>`，复用现有 release 与 root_deploy，不新增发布平台或生产开发镜像。
一次构建/传输后在 Windows 本地完成严格校验和安装；独立锁定 Python 避免从正被更新的根运行安装器。
未变化的管理文件不重写、不备份，移除文件仍保留恢复所需旧内容。
工具更新包与含运行时的完整安装包分离：锁和安装清单一致时只传工具包，绑定准确运行时 hash；首次安装/依赖变化使用完整包。
本增量不另建通用内容寻址运行时仓库；已有锁定依赖缓存继续复用。
成功后保留最近两次回滚和当前及前两版登记安装产物，释放本次 staging；仅新机制登记的完整成功副本可以自动淘汰。
失败/未知/修改过的副本及旧机制历史不按年龄删除，未完成事务不清理；Work、资产、接受/Final 与浏览器缓存不在此部署回收范围。
默认随成功部署清理，不建定时服务；历史存量清理需另行授权。原生验证与工具测试分别记录，入口实现不等于已更新生产。
